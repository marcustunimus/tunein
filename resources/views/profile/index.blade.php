        ...
        @foreach ($posts as $post)
            <x-post.panel profilePictureURL="{{ $post->author->profile_picture }}" profileName="{{ $post->author->username }}" contentId="postContent{{ $post->id }}" timePassed="{{ $post->created_at->diffForHumans() }}">
                ...
                <x-post.interaction.tab>
                    <x-post.interaction.button id="post-{{ $post->id }}-like" title="{{ $post->isLikedByUser($user) ? 'Unlike' : 'Like' }}" icon="{{ $post->isLikedByUser(auth()->user()) ? 'background-image: url(' . asset('/images/favorite_white_24dp.svg') . ');' : 'background-image: url(' . asset('/images/favorite_border_white_24dp.svg') . ');' }}"></x-post.interaction.button>
                    ...
                    <x-post.interaction.button id="post-{{ $post->id }}-bookmark" title="{{ $post->isBookmarkedByUser($user) ? 'Remove ' : '' }}Bookmark" icon="{{ $post->isBookmarkedByUser(auth()->user()) ? 'background-image: url(' . asset('/images/bookmark_white_24dp.svg') . ');' : 'background-image: url(' . asset('/images/bookmark_border_white_24dp.svg') . ');' }}"></x-post.interaction.button>
                </x-post.interaction.tab>
